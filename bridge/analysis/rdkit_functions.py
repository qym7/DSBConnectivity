...
try:
    ...
    from rdkit.Chem import RDConfig, QED, Crippen, AllChem, MolFromSmiles, MolToSmiles, Descriptors
...
class BasicMolecularMetrics(object):
    ...
    def compute_relaxed_validity(self, generated):
        ...
        return valid_smiles, all_smiles, len(valid_smiles) / len(generated)
    ...
    def compute_sascore(self, all_smiles):
        ...
        return v_sa_avg, sa_values, count_true_sa / len(all_smiles)
    ...
    def check_nll(self, generated, source, eps=1e-18):
        ...
        for (X_0, E_0), (X_T, E_T) in zip(generated, source):
            ...
            E_0_pad = torch.zeros((max_nodes_0, max_nodes_0), dtype=E_0.dtype, device=E_0.device)
            E_T_pad = torch.zeros((max_nodes_T, max_nodes_T), dtype=E_T.dtype, device=E_T.device)
            ...
            # create node masks
            node_masks.append((torch.arange(max(max_nodes_0, max_nodes_T), device=X_0.device) < n_nodes_0).float())
        ...
        n0 = torch.tensor([X.shape[0] for X, _ in generated], device=X_0_padded.device).unsqueeze(1)
        nT = torch.tensor([X.shape[0] for X, _ in source], device=X_T_padded.device).unsqueeze(1)
        ...
        K = pygm.utils.build_aff_mat(X_0_padded.unsqueeze(-1), edge0, conn0,
                                     X_T_padded.unsqueeze(-1), edgeT, connT,
                                     n0, ne0, nT, neT,
        ...
        X_T_aligned = torch.matmul(Y, X_T_padded.unsqueeze(-1)).squeeze(-1)  # Align node values
        ...
        nll_E = nll_E * node_masks.unsqueeze(-1) * node_masks.unsqueeze(-2)  # Mask edge values
    ...
    def evaluate(self, generated, source):
        ...
        if source is not None:
            ...
            v_sa_avg_source, sa_values_source, sa_v_source = self.compute_sascore(all_relaxed_smiles_source)
        else:
        ...
        relaxed_valid_smiles, all_smiles, relaxed_validity = self.compute_relaxed_validity(generated)
        print(
        ...
        if relaxed_validity > 0:
            ...
            print(f"SA Average Value over {len(relaxed_valid_smiles)} unique valid molecules: {v_sa_avg :.2f}")
        ...
        return (
            [validity, relaxed_validity, uniqueness, novelty, connectivity, self.train_sa_avg, self.train_sa_success, self.test_sa_avg, self.test_sa_success, coverage, v_sa_avg, v_sa, vu_sa, vun_sa],
            unique_smiles,
    ...
    def evaluate_zinc(self, generated, source):
        ...
        if source is not None:
            ...
            print(f"NLL Score over {len(generated)} total generated molecules is: {nll}")
        else:
            valid_source = None
        ...
        if relaxed_validity > 0:
            ...
            if valid_source is not None:
                ...
                logP_source, qed_source, sa_source = self.compute_properties(valid_source)

                w1_logP = self.compute_w1(logP_gen, logP_source)
                print(f"W1 LogP over {len(relaxed_valid)} valid molecules is: {w1_logP}")
                mad_qed, qed_length = self.compute_mad(qed_gen, qed_source)
                print(f"MAD QED over {qed_length} (a portion) valid molecules is: {mad_qed}")
                ...
                print(f"FCD Score over {len(relaxed_valid)} valid molecules is: {fcd_score}")
            else:
                w1_logP = -1.0
                mad_qed = -1.0
                mad_sa = -1.0
            ...
            coverage = 0.0
            unique = []
        return ([validity, relaxed_validity, uniqueness, novelty, connectivity,
             w1_logP, mad_qed, mad_sa, fcd_score, coverage, nll], unique,
            dict(nc_min=nc_min, nc_max=nc_max, nc_mu=nc_mu),
    ...
    def evaluate_baselines(self, generated):
        ...
        if relaxed_validity > 0:
            ...
            print(f"SA Score Success Rate (<3) over {len(unique)} unique valid molecules: {vu_sa * 100 :.2f}%")
            print(
                f"SA Score Success Rate (<3) over {len(novel)} novel unique valid molecules: {vun_sa * 100 :.2f}%")
...
def compute_molecular_metrics(molecule_list, test_smiles, train_smiles, dataset_info, fb, source_molecule_list):
    ...
    dic = {
        ...
        f"mol_metrics_charts_{fb}/V.U.N.": rdkit_metrics[0][1] * rdkit_metrics[0][2] * rdkit_metrics[0][3],
        f"mol_metrics_charts_{fb}/nc_max": nc["nc_max"],
        f"mol_metrics_charts_{fb}/nc_mu": nc["nc_mu"],
    }
    if wandb.run:
