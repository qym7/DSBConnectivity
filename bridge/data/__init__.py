...
def get_dataloader(d_config, data_init, data_folder, bs, training=False):
    ...
    kwargs = {
        "batch_size": bs,
        "shuffle": True,
        "num_workers": d_config.num_workers,
        "drop_last": True,
    }
    if training:
...
def get_dataset(d_config, data_folder):
    ...
    if d_config.dataset == "CIFAR10":
        ...
        dataset = CIFAR10(
            path, train=True, download=True, transform=cmp(train_transform)
        )
        test_dataset = CIFAR10(
            path, train=False, download=True, transform=cmp(test_transform)
        )
        ...
        dataset = CelebA(
            path, split="train", transform=cmp(train_transform), download=True
        )
        test_dataset = CelebA(
            path, split="test", transform=cmp(test_transform), download=True
        )
    ...
    #                                  source_root=data_folder, train=False)
        ...
        path = data_folder
        dataset = LSUN(
            path, classes=[d_config.category + "_train"], transform=cmp(train_transform)
        )
        test_dataset = LSUN(
            path, classes=[d_config.category + "_val"], transform=cmp(test_transform)
        )
