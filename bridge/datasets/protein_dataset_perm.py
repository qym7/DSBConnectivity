...
class ProteinDataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        if self.split == "train":
            return [
                ...
                f"train_perm.pt",
                f"train_perm_n.pickle",
                f"train_perm_node_types.npy",
                f"train_perm_bond_types.npy",
            ]
        elif self.split == "val":
            return [
                f"val.pt",
                ...
                f"val_perm.pt",
                f"val_perm_n.pickle",
                f"val_perm_node_types.npy",
                f"val_perm_bond_types.npy",
            ]
        else:
            return [
                f"test.pt",
                ...
                f"test_perm.pt",
                f"test_perm_n.pickle",
                f"test_perm_node_types.npy",
                f"test_perm_bond_types.npy",
            ]
    ...
    def process(self):
        ...
        for idx in indices:
            ...
            perm_data = torch_geometric.data.Data(
                x=nodes,
            ...
            if self.pre_filter is not None and not self.pre_filter(perm_data):
                continue
            if self.pre_transform is not None:
                perm_data = self.pre_transform(perm_data)
            perm_data_list.append(perm_data)

            # get original data
            nodes = data_node_label[node_idx].long()
            edge_idx = node_idx[data_adj[:, 0]]
            edge_index = data_adj[edge_idx] - offset
            edge_attr = torch.ones_like(edge_index[:, 0]).long()
        ...
        node_types = atom_type_counts(perm_data_list, num_classes=self.num_node_type)
        bond_types = edge_counts(perm_data_list, num_bond_types=self.num_edge_type)
        torch.save(self.collate(perm_data_list), self.processed_paths[4])
        save_pickle(num_nodes, self.processed_paths[5])
        np.save(self.processed_paths[6], node_types)
        np.save(self.processed_paths[7], bond_types)
