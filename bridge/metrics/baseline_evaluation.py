...
def nll_graph_edit_distance(G1, G2, alpha):
    ...
    for i in range(len(G1['nodes']), dV):
        for j in range(len(G2['nodes']), dV):
            node_cost_matrix[i, j] = compute_cost_alpha(dV, alpha)[1]  # Cost for dummy nodes
...
if __name__ == "__main__":
    ...
    metrics = BasicMolecularMetrics(dataset_info=dataset_info, test_smiles=optimized_smiles, train_smiles=original_smiles)
    valid_optimized, _, _, _, valid_indices = metrics.evaluate_baselines(optimized_smiles)
